...
bool c_keys::InitKeyboard( )
{
	...
	else
	{
		...
		if ( gafAsyncKeyStateExport < 0x7FFFFFFFFFFF )
		{
			...
			auto result = VMMDLL_Map_GetModuleFromNameW( mem.vHandle, mem.GetPidFromName( "winlogon.exe" ) | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, static_cast< LPWSTR >( const_cast< wchar_t* >( L"win32kbase.sys" ) ), &module_info, VMMDLL_MODULE_FLAG_NORMAL );
			if ( !result )
			{
